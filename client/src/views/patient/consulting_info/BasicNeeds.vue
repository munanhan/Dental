        ...
        >
            <div class="content">
                <el-table
                        ...
                        :data="basic_need_menu.data"
                        :header-cell-style="{backgroundColor:'#e3e3e3',color:'#3a3a3a'}"
                        :height="tableHeight"
                >
                    <el-table-column type="index"></el-table-column>
                    <el-table-column
                            v-for="(item,k) in basic_need_menu.columns"
                            :key="k"
                            :prop="item.field"
                            :label="item.title"
                    >
                        <template slot-scope="scope">
                            <span v-if="scope.row.isSet">
                                <el-input size="mini" placeholder="请输入内容" v-model="basic_need_menu.sel[item.field]">
                                </el-input>
                            </span>
    ...
    methods: {
        ...
        getTeethHabitList(){
            ...
            that.$api.teethHabit
                .get()
                .then(res => {
                    that.basic_need_menu.data.map(i => {
                        i.id = res.data.id;
                        i.isSet=false;
                        return i;
                    });
                    that.basic_need_menu.data = res.data;
                })
                .catch(res => {
                    console.log(res);
                });
        ...
        addTeethHabitItem() {
            let that=this;
            for (let i of that.basic_need_menu.data) {
                if (i.isSet) return that.$message.warning("请先保存当前编辑项");
            }
            let j = { "name": "",  "isSet": true,};
            that.basic_need_menu.data.push(j);
            that.basic_need_menu.sel = JSON.parse(JSON.stringify(j));
        },
            ...
            that.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                .then(() => {
                    that.$api.teethHabit.del({id})
                        .then(res => {
                            if (res.code == 200) {
                                that.basic_need_menu.data.splice(index, 1);
                                that.$message({
                                    type: 'success',
                                    message: '删除成功!'
                                });
        ...
        store(row,index){
            let that=this;
            let data = JSON.parse(JSON.stringify(that.basic_need_menu.sel));
        ...
        edit(row,index){
            ...
            that.basic_need_menu.sel = JSON.parse(JSON.stringify(row));
            that.$set(row,'isSet',true);
        },

        cancel(row,index){
            let that=this;
            if (!that.basic_need_menu.sel.id) that.basic_need_menu.data.splice(index, 1);
            row.isSet = false;
        },
