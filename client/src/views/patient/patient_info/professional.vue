        ...
        >
            <div class="content">
                <el-table
                    ...
                    :data="professional_menu.data"
                    :header-cell-style="{backgroundColor:'#e3e3e3',color:'#3a3a3a'}"
                    :height="tableHeight"
                >
                    <el-table-column type="index"></el-table-column>
                    <el-table-column
                        v-for="(item,k) in professional_menu.columns"
                        :key="k"
                        :prop="item.field"
                        :label="item.title"
                    >
                        <template slot-scope="scope">
                            <span v-if="scope.row.isSet">
                                <el-input
                                    size="mini"
                                    placeholder="请输入内容"
                                    v-model="professional_menu.sel[item.field]"
                                >
                                </el-input>
                            </span>
...
export default {
    ...
    methods: {
        getprofessionalList() {
            ...
            that.$api.patient_professional
                ...
                .then(res => {
                    ...
                    that.professional_menu.data = res.data;
                })
                .catch(res => {
                    console.log(res);
                });
        ...
        store(row, index) {
            let that = this;
            let data = JSON.parse(JSON.stringify(that.professional_menu.sel));
        ...
        edit(row, index) {
            ...
            that.professional_menu.sel = JSON.parse(JSON.stringify(row));
            that.$set(row, "isSet", true);
        },

        cancel(row, index) {
            let that = this;
            if (!that.professional_menu.sel.id)
                that.professional_menu.data.splice(index, 1);
            row.isSet = false;
        }
    }
