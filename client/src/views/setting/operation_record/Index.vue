<template>
  <div ref="menuContent" class="content">
    <el-container>
        ...
        <el-row class="search" style="border-bottom: 1px solid rgba(33, 37, 43, 0.26);padding-bottom: 30px;">
      ...
      <el-main class="right" style="padding: 0;">
        <el-row class="right_top">
          {{show_time.dtfm}} - {{show_time.dtto}}(共{{show_time.total_day}}天)
        </el-row>
    ...
    components: {},
      ...
      methods: {
        ...
        calculation(){
          ...
          this.show_time.total_day = (dtto-dtfm+(60 * 60 * 24 * 1000))/(60 * 60 * 24 * 1000);//计算天数
        ...
        prev(){
          ...
          var yesterday = this.search.dtfm == ''?new Date(new Date - 1000 * 60 * 60 * 24):new Date(date - 1000 * 60 * 60 * 24);
          switch(option){
            case 1:
            //获取上一天
        ...
        next(){
          ...
          var tomorrow = this.search.dtto == ''?new Date(new Date + 1000 * 60 * 60 * 24):new Date(date + 1000 * 60 * 60 * 24);
          switch(option){
            case 1:
            //获取下一天
        ...
        getData() {
          let that = this;
          //表单数据
          that.$api.operation_log.get(that.search)
            .then(res => {
        ...
        getUserData(){
          //用户下拉
          let that = this;
          that.$api.operation_log.get_user_list()
          .then(res => {
        ...
        getModuleData(){
          //模块列表
          let that = this;
          that.$api.operation_log.get_module_list()
          .then(res => {
